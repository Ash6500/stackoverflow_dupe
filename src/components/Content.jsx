...
const Content = ({ searchQuery }) => {
  ...
  const fetchQuestions = async (filter, query = "") => {
    setLoading(true);
    try {
  ...
  // Fetch questions when active category or search query changes
  useEffect(() => {
    const category = categories.find((c) => c.label === activeCategory);
    fetchQuestions(category.filter, searchQuery);
  ...
  return (
    <div className="max-w-3xl mx-auto bg-white">
      ...
      <div className="p-4 border-b flex items-center">
        ...
        <button className="ml-8 text-sm font-medium bg-blue-500 text-white px-4 py-3 rounded-full">
          Ask Question
        </button>
      </div>
